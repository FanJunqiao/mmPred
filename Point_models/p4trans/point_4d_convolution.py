...
class P4DConv(nn.Module):
    ...
    def forward(self, xyzs: torch.Tensor, features: torch.Tensor = None) -> (torch.Tensor, torch.Tensor):
        ...
        # xyzs_all = torch.cat(xyzs, dim = 1)
        # print(xyzs_all.shape)
        # anchor_idx = pointnet2_utils.furthest_point_sample(xyzs_all, npoints//self.spatial_stride)                               # (B, N//self.spatial_stride)
        # anchor_xyz_flipped = pointnet2_utils.gather_operation(xyzs_all.transpose(1, 2).contiguous(), anchor_idx)                 # (B, 3, N//self.spatial_stride)
        ...
        # print("point", xyzs_all.shape, anchor_xyz.shape)
        # # from plot_regress import plot_points_and_anchors_with_circles
        # # plot_points_and_anchors_with_circles(xyzs_all.cpu().numpy(), anchor_xyz.cpu().numpy(), save_path=f"./debug/point{0}.png")
