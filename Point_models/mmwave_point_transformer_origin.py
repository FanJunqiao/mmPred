...
class TransitionDown(nn.Module):
    def __init__(self, in_channel, internal_channel, out_channel):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.Linear(in_channel, internal_channel),
            nn.ReLU()
        )
        self.conv2 = nn.Sequential(
            nn.LayerNorm(internal_channel),
            nn.Linear(internal_channel, internal_channel),
            nn.ReLU()
        )
...
class PointTransformerReg(nn.Module):
    def __init__(self, input_dim = 5, nblocks = 5, nneighbor = 16, transformer_dim = 128, n_p = 17, n_pre = 4):
        super().__init__()
        ...
        dim, depth, heads, dim_head, mlp_dim, dropout = 512, 5, 4, 128, 256, 0.1
        ...
        self.xyzt_pos_emb = nn.Conv1d(in_channels=4, out_channels=dim, kernel_size=1, stride=1, padding=0, bias=True)
