...
class Diffusion:
    ...
    def inpaint_complete(self, step, x, prev_t, traj_dct, mode_dict, traj_dct_cond):
        """
        ...
        if mode_dict['mode'] == 'control':
            if self.dct_enable:
                ...
                traj_dct_fix = mode_dict['traj_fix']
        ...
        if mode_dict['mode'] == 'switch':
            if step < 20:  # relax to make switch smoother
                ...
                if self.dct_enable:
                   traj_dct_switch = torch.matmul(self.dct[:self.n_pre], mode_dict['traj_switch'])  # transform to DCT domain for adding noise
                else:
                    traj_dct_fix = mode_dict['traj_switch']
    ...
    # def inpaint_complete1(self, step, x, prev_t, traj_dct, mode_dict, traj_dct_cond):
    #     """
    #     perform mask completion
    ...
    #     else:
    #         x_prev_t_known, _ = self.noise_motion(traj_dct, prev_t)  # add noise in DCT domain
    #         # x_prev_t_known, _ = traj_dct, 0

    #     x_prev_t_known = torch.matmul(self.idct[:, :self.n_pre],
    #                                   x_prev_t_known[:, :self.n_pre])  # return time domain for mask
    #     x_prev_t_unknown = torch.matmul(self.idct[:, :self.n_pre],
    #                                     x[:, :self.n_pre])
    #     x_cond = torch.matmul(self.idct[:, :self.n_pre_cond],
    #                                   traj_dct_cond[:, :self.n_pre_cond])  # return time domain for mask
        
    #     # x = x_prev_t_unknown
    #     x = torch.mul(mode_dict['mask'], x_prev_t_known) + torch.mul((1 - mode_dict['mask']), x_prev_t_unknown)  # mask
    #     x_cond_new = torch.mul(mode_dict['mask'], x_prev_t_unknown) + torch.mul((1 - mode_dict['mask']), x_cond)  # mask

    #     if step > 900:  # relax to make switch smoother
    #         x_cond = x_cond_new
    ...
    #     x = torch.matmul(self.dct[:self.n_pre], x)
    #     x_cond = torch.matmul(self.dct[:self.n_pre_cond], x_cond)

    #     return x, x_cond

    def sample_ddim_progressive(self, model, traj_dct, traj_dct_mod, mode_dict, noise=None, gpu_split = True):
        """
        ...
        if noise is not None: 
            '''
            Currently implement for cond denoising.
            '''
            prev_t = (torch.ones(sample_num) * self.ddim_timestep_prev_seq[self.refine_timesteps-1]).long().to(self.device)
            noise, _ = self.noise_motion(noise, prev_t)  # add noise in DCT domain
            x = noise
            EnableComplete = False
            timesteps = self.refine_timesteps
            # x = torch.randn((sample_num, self.motion_size[0], self.motion_size[1])).to(self.device)
            # x = torch.matmul(self.idct[:, :self.n_pre],
            #                           x[:, :self.n_pre])  # return x to time domain for mask
            # prev_t = (torch.ones(sample_num) * self.ddim_timestep_prev_seq[self.ddim_timesteps-1]).long().to(self.device)
            # noise, _ = self.noise_motion(noise, prev_t)  # add noise in DCT domain
            # x_prev_t_known = torch.matmul(self.idct[:, :self.n_pre],
            #                           noise[:, :self.n_pre])  # return noise to time domain for mask
            # x = torch.mul(mode_dict['mask'], x_prev_t_known) + torch.mul((1 - mode_dict['mask']), x)  # mask
            # x = torch.matmul(self.dct[:self.n_pre], x)
            # print("hi", x.min(), x.max())
        else:
            x = torch.randn((traj_dct.shape[0], traj_dct.shape[1], traj_dct.shape[2])).to(self.device)
            # x = torch.zeros((sample_num, self.motion_size[0], self.motion_size[1])).to(self.device)
        ...
        with torch.no_grad():
            for i in reversed(range(0, timesteps)):
                ...
                # split for gpu consumption
                if gpu_split:
                    predicted_noise_list = []
                    split_num = 4
                    off_set = sample_num//split_num
                    for i in range(split_num-1):
                        start = off_set * i 
                        end = off_set * (i+1)
                        x1 = x[start:end, ...]
                        t1 = t[start:end, ...]
                        traj_dct_mod1 = traj_dct_mod[start:end, ...]
                        predicted_noise_list.append(model(x1, t1, mod=traj_dct_mod1))
                    x2 = x[end:, ...]
                    t2 = t[end:, ...]
                    traj_dct_mod2 = traj_dct_mod[end:, ...]
                    predicted_noise_list.append(model(x2, t2, mod=traj_dct_mod2))
