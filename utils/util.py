...
def get_dct_dev1_matrix(N, is_torch=True):
    dct_m = np.eye(N)
    idct_m = np.eye(N)
    aug_m = np.eye(N)
    for k in np.arange(N):
        for i in np.arange(N):
            ...
            if k == 0:
                w = np.sqrt(1 / N)
            dct_m[k, i] = w * np.cos(np.pi * (i + 1 / 2) * k / N) * (-k * np.pi / N)
            idct_m[k,i] = w * np.sin(np.pi * (i + 1 / 2) * k / N)
            aug_m[k,k] = (-k * np.pi / N)
    # idct_m = np.linalg.inv(dct_m)
    if is_torch:
        dct_m = torch.from_numpy(dct_m)
        idct_m = torch.from_numpy(idct_m)
        aug_m = torch.from_numpy(aug_m)
    return dct_m, idct_m, aug_m

def get_dct_dev2_matrix(N, is_torch=True):
    dct_m = np.eye(N)
    idct_m = np.eye(N)
    for k in np.arange(N):
        for i in np.arange(N):
            ...
            if k == 0:
                w = np.sqrt(1 / N)
            dct_m[k, i] = w * np.cos(np.pi * (i + 1 / 2) * k / N) * (-(k * np.pi / N))
            idct_m[k,i] = w * np.cos(np.pi * (i + 1 / 2) * k / N) * (k * np.pi / N)
    # idct_m = np.linalg.inv(dct_m)
    if is_torch:
        dct_m = torch.from_numpy(dct_m)
        idct_m = torch.from_numpy(idct_m)
...
def cart_to_vel(traj):
    traj_clone = traj.clone()
    traj_vel= ((traj_clone[...,1:,:] - traj_clone[...,:-1,:]))
    
    # traj_vel /= 0.3
    # traj_vel *= 100

    traj_vel = torch.cat([torch.zeros_like(traj_clone[...,:1,:]), traj_vel],dim=-2)
    
    return traj_vel

def vel_to_cart(traj_vel, traj_start):


    traj_vel_clone = traj_vel.clone()
    if len(traj_vel.shape) == 3:
        t_steps = traj_vel.shape[-2]
        traj_est = traj_start.clone().repeat(1, t_steps, 1)
    elif len(traj_vel.shape) == 4:
        t_steps = traj_vel.shape[-2]
        traj_est = traj_start.clone().repeat(1, 1, t_steps, 1)
    for i in range(t_steps-1):
        traj_est[...,(i+1):,:] += ((traj_vel_clone[...,[i+1],:]))
