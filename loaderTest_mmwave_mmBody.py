...
from depos_utils.h36m import H36M
...
from models.mmPred_transformer import MotionTransformer

from models.PSGCN.model import GCN

from models.mamba_simple_kalman import ModelArgs ### no robust config
from models.MambaTransendBase_trans_time_drop import Mamba ###  test implementation
...
def setup():
    ...
    parser.add_argument('--cfg',
                        default='mmBody', help='h36m or humaneva')
    ...
    parser.add_argument('--ckpt', type=str, default='./results_paper/train_mmBody_final_0/models/ckpt_ema_250.pt')
...
if __name__ == '__main__':
    ...
    if args.diffusion == True:
        ...
        #     num_frames=(cfg.t_his + cfg.t_pred) if cfg.dct_enable == False else cfg.n_pre,
        ...
        # ).to(cfg.device)
        expand = 1
        mambaArgs = ModelArgs(d_model= 384// expand,
    ...
    if args.mode.split("_")[0] == "test":
        ...
        if args.diffusion == True:
            np.random.seed(20)
            plot_indexes = np.random.choice(range(3000,4000), size=30, replace=False)[20:]
            
            ckpt_rgb = torch.load("./results_final/train_mmBody_rgb_170/models/ckpt_ema_300.pt")
            model_rgb = MotionTransformer(
                input_feats=3 * cfg.joint_num,  # 3 means x, y, z
                num_frames=(cfg.t_his + cfg.t_pred) if cfg.dct_enable == False else cfg.n_pre,
                num_layers=cfg.num_layers,
                num_heads=cfg.num_heads,
            ...
            model_rgb.eval()
            print("Model rgb complete.")
            ckpt_baseline = torch.load("./results_final/train_mmBody_baseline_203/models/ckpt_ema_500.pt")
            model_baseline = MotionTransformer(
                input_feats=3 * cfg.joint_num,  # 3 means x, y, z
                num_frames=(cfg.t_his + cfg.t_pred) if cfg.dct_enable == False else cfg.n_pre,
                num_layers=cfg.num_layers,
                num_heads=cfg.num_heads,
            ...
            model_baseline.load_state_dict(ckpt_baseline)
            model_baseline.eval()
            print("Model baseline complete.")
            
            if not os.path.exists("test_vis_mmBody3"):
                os.makedirs("test_vis_mmBody3")
            
            demo_visualize(args.mode, cfg, model, diffusion, [test_dataset_list[0]], plot_indexes=plot_indexes, name="my", plot_pc=True) #4600, 1723
            # demo_visualize(args.mode, cfg, model_rgb, diffusion, [test_dataset_list[0]], plot_indexes=plot_indexes, name="rgb") #4600, 1723
            # demo_visualize(args.mode, cfg, model_baseline, diffusion, [test_dataset_list[1]], plot_indexes=plot_indexes, name="baseline", plot_pc=True) #4600, 1723
            # demo_visualize(args.mode, cfg, model_baseline, diffusion, [test_dataset_list[1]], plot_indexes=plot_indexes, name="gt", plot_pc=False) #4600, 1723
            compute_stats(diffusion, [test_dataset_list[0]], model, logger, cfg, multiFlag=args.multiFlag)
